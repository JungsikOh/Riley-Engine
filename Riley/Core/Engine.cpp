...
Engine::Engine(EngineInit const& init) : window(init.window), vsync{init.vsync}
{
    ...
    std::ignore = inputEvents.left_mouse_clicked.Add([this](uint32 mx, uint32 my) { renderer->OnLeftMouseClicked(mx, my); });
    ...
    modelImporter->LoadModel("Resources/Models/DamagedHelmet/", "DamagedHelmet.gltf", false, Vector3(0.0f), 0.5f);
    ...
    modelImporter->LoadLight(light, LightMesh::Quad, 150.0f);
