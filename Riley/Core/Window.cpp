...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM w_param, LPARAM l_param) {
    ...
    window_data.width =
        this_window ? static_cast<float>(this_window->Width()) : 0.0f;
    window_data.height =
        this_window ? static_cast<float>(this_window->Height()) : 0.0f;
...
Window::Window(WindowInit const& init) {
   ...
   const std::wstring window_title = ToWideString(init.title);
    ...
    if (!RegisterClassExW(&wcex))
        MessageBoxA(nullptr, "Window class registration failed!",
                    "Fatal Error!", MB_ICONEXCLAMATION | MB_OK);

    hwnd = CreateWindowExW(0, class_name, window_title.c_str(),
                           WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT,
                           window_width, window_height, nullptr, nullptr,
                           hinstance, nullptr);

    if (!hwnd) {
        MessageBox(nullptr, L"Window creation failed!", L"Fatal Error!",
...
// multicast delegate¸¦ ÅëÇØ Á¤ÀÇÇÑ WindowEvent Å¬·¡½ºÀÇ µî·ÏµÈ ÄÝ¹éÀ» ¸ðµÎ
// È£ÃâÇÑ´Ù.
