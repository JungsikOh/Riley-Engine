...
Camera::Camera(CameraParameters const& desc)
    : position{desc.position}, lookVector{desc.lootAt}, fov{desc.fov}, aspectRatio{desc.aspectRatio}, nearPlane{desc.nearPlane},
...
{
    Matrix rotate = Matrix::CreateFromAxisAngle(rightVector, sensitivity * DirectX::XMConvertToRadians((float)dy));
...
{
    Matrix rotate = Matrix::CreateRotationY(sensitivity * DirectX::XMConvertToRadians((float)dx));
...
{
    projRow = DirectX::XMMatrixPerspectiveFovLH(DirectX::XMConvertToRadians(fov), aspect, zn, zf);
}
