...
std::vector<entt::entity> ModelImporter::LoadSquare(const Vector3& pos,
    ...
    mesh.vertexBuffer = std::make_shared<DXBuffer>(
        m_device, VertexBufferDesc(vertices.size(), sizeof(Vertex)),
        vertices.data());
...
ModelImporter::LoadBox(const Vector3& pos, const float& scale /*1.0f*/,
...
std::vector<entt::entity> ModelImporter::LoadLight(Light& lightData,
                                                   LightMesh meshType,
                                                   const float& scale) {
    ...
    if (meshType == LightMesh::Quad) {
        ...
        Mesh mesh{};
        mesh.vertexBuffer = std::make_shared<DXBuffer>(
            m_device, VertexBufferDesc(vertices.size(), sizeof(SimpleVertex)),
            vertices.data());
        mesh.indexBuffer = std::make_shared<DXBuffer>(
            m_device, IndexBufferDesc(indices.size(), true), indices.data());
        mesh.indexCount = static_cast<uint32>(indices.size());
        ...
        Matrix translation =
            DirectX::XMMatrixTranslationFromVector(lightData.position);
