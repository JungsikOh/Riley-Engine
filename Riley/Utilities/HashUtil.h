...
namespace Riley {
template <typename T> constexpr void HashCombine(size_t& seed, T const& v) {
    std::hash<T> hasher{};
...
struct CrcInternal
    : CrcInternal<((c & 1) ? 0xd800000000000000L : 0) ^ (c >> 1), k - 1> {};
...
template <size_t N> consteval uint64_t crc64(char const (&_str)[N]) {
