...
namespace Riley {
#define DECLARE_DELEGATE(name, ...) using name = Delegate<void(__VA_ARGS__)>
...
template <typename R, typename... Args> class Delegate<R(Args...)> {
  ...
  public:
    ...
    template <
        typename F,
        std::enable_if_t<std::is_constructible_v<DelegateType, F>>* = nullptr>
#endif
    ...
    void BindMember(R (T::*mem_pfn)(Args...), T& instance) {
        callback = [&instance, mem_pfn](Args&&... args) mutable -> R {
            return (instance.*mem_pfn)(std::forward<Args>(args)...);
        };
    }

    void UnBind() { callback = nullptr; }

    R Execute(Args&&... args) { return callback(std::forward<Args>(args)...); }
...
template <typename... Args> class MultiCastDelegate {
  ...
  public:
    ...
    [[maybe_unused]] DelegateHandle Add(F&& callable) {
        delegate_array.emplace_back(DelegateHandle(0),
                                    std::forward<F>(callable));
        return delegate_array.back().first;
    }

    template <typename T>
    [[maybe_unused]] DelegateHandle AddMember(void (T::*mem_pfn)(Args...),
                                              T& instance) {
        delegate_array.emplace_back(
            DelegateHandle(0),
            [&instance, mem_pfn](Args&&... args) mutable -> void {
                return (instance.*mem_pfn)(std::forward<Args>(args)...);
            });
        return delegate_array.back().first;
    }
    ...
    void Broadcast(Args... args) {
        for (size_t i = 0; i < delegate_array.size(); ++i) {
            if (delegate_array[i].first.IsValid())
                delegate_array[i].second(std::forward<Args>(args)...);
        }
    }
