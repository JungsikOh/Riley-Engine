...
public:
    ...
    DXRenderTarget(ID3D11Device* device, UINT width, UINT height, DXFormat format = DXFormat::R8G8B8A8_UNORM,
                   DXDepthStencilBuffer* buffer = nullptr);
    DXRenderTarget(ID3D11Device* device, ID3D11RenderTargetView* rtv, DXDepthStencilBuffer* buffer = nullptr);
    ...
    ID3D11RenderTargetView* RTV(uint64 i = 0) const {
        return m_RTVs[i];
    }
    ...
    void Initalize(ID3D11Device* device, UINT width, UINT height, DXFormat format = DXFormat::R8G8B8A8_UNORM,
                   DXDepthStencilBuffer* buffer = nullptr);

    void CreateRenderTarget(ID3D11Device* device, DXFormat format = DXFormat::R8G8B8A8_UNORM,
