...
namespace Riley
{
  ...
  public:
    ...
    bool BindSRV(ID3D11DeviceContext* context, unsigned int bindSlot,
    ...
    HRESULT
    CreateSRV(ID3D11Device* device, D3D11_SHADER_RESOURCE_VIEW_DESC* desc)
    {
      if(m_srv == nullptr)
        {
    ...
    HRESULT
    CreateUAV(ID3D11Device* device, D3D11_UNORDERED_ACCESS_VIEW_DESC* desc)
    {
      if(m_uav == nullptr)
        {
