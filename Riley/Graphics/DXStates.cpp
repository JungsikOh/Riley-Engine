...
constexpr D3D11_TEXTURE_ADDRESS_MODE ConvertTextureAddressMode(DXTextureAddressMode mode)
{
   switch (mode)
      {
...
DXRasterizerState::DXRasterizerState(ID3D11Device* device, DXRasterizerStateDesc const& desc)
{
   D3D11_RASTERIZER_DESC d3d11_desc = ConvertRasterizerDesc(desc);
   HR(device->CreateRasterizerState(&d3d11_desc, &rasterizerState));
}

DXDepthStencilState::DXDepthStencilState(ID3D11Device* device, DXDepthStencilStateDesc const& desc)
{
