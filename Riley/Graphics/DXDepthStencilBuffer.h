...
class DXDepthStencilBuffer : public DXResource {
  public:
    ...
    DXDepthStencilBuffer(ID3D11Device* device, UINT width, UINT height,
                         bool isStencilEnable = true);
    virtual ~DXDepthStencilBuffer() { SAFE_RELEASE(m_dsv); }

    virtual DXRenderResourceType GetResourceType() const override {
        return DXRenderResourceType::DepthStencilBuffer;
    }
